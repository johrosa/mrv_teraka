...
class MerginDataMerger:
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            # 2. Batch UPSERT: Combiner les ajouts et modifications
            # PostgREST supporte l'upsert en batch avec Prefer: resolution=merge-duplicates
            items_to_upsert = [item for item in collected if item.get(pk_field) in added_ids or item.get(pk_field) in modified_ids]
            if items_to_upsert:
                try:
                    # insert(upsert=True) utilise le header approprié
            ...
            # 3. Batch DELETE: Utiliser l'opérateur in. pour supprimer en une seule requête
            if deleted_ids:
                try:
