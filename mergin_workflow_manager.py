...
class MerginDataMerger:
    ...
    def detect_conflicts(self, original, collected, pk_field='id'):
        ...
        # Entrées modifiées - Optimisé O(N+M) avec dictionnaire
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            # 2. Traiter les ajouts et modifications en batch (UPSERT) - Optimisé O(1) requêtes
            ...
            # 3. Traiter les suppressions en batch - Optimisé O(1) requêtes
            deleted_ids = []
            for c in conflicts:
                if c['type'] == 'deleted':
            ...
            if deleted_ids:
                try:
                    # Conversion en string pour l'opérateur 'in.' de PostgREST
