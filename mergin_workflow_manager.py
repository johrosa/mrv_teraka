...
class MerginDataMerger:
    ...
    def detect_conflicts(self, original, collected, pk_field='id'):
        ...
        # Entrées modifiées - Optimization: O(N) lookup using dictionary
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            # 2. Optimization: Batch UPSERT for added and modified records
            ...
            # 3. Optimization: Batch DELETE for deleted records using 'in.' operator
            deleted_ids = []
            for c in conflicts:
                if c['type'] == 'deleted':
            ...
            if deleted_ids:
                try:
                    # PostgREST batch delete: id=in.(val1,val2,...)
