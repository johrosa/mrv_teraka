...
class MerginDataMerger:
    ...
    def detect_conflicts(self, original, collected, pk_field='id'):
        ...
        # Entrées modifiées - Optimized O(N+M) using dict lookup instead of O(N*M) nested search
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            # 2. Traiter les ajouts et modifications en une seule requête UPSERT (batch)
            items_to_upsert = [item for item in collected if item.get(pk_field) in added_ids or item.get(pk_field) in modified_ids]
            if items_to_upsert:
                try:
                    # Optimized: one batch request for both new and modified items
            ...
            # 3. Traiter les suppressions en une seule requête DELETE (batch)
            if deleted_ids:
                try:
                    # Optimized: one batch request using 'in.' operator
