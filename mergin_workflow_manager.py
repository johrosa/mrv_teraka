...
class MerginDataMerger:
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            # 2. Traiter les ajouts et modifications en une seule opération UPSERT (si supporté par l'API)
            # Bolt: Optimisation batch pour réduire les roundtrips réseau
            ...
            # 3. Traiter les suppressions en batch (chunked pour éviter URL trop longue)
            # Bolt: Optimisation batch DELETE avec chunking (limite URL PostgREST)
            chunk_size = 200
