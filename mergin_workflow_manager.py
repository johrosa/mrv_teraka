...
class MerginDataMerger:
    ...
    def detect_conflicts(self, original, collected, pk_field='id'):
        ...
        # Entrées modifiées (Optimisation: O(N+M) avec dictionnaire au lieu de O(N*M))
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            # 1. Identifier les changements (Optimisation: Batch UPSERT et Batch DELETE)
            ...
            # 2. Traiter AJOUTS et MODIFICATIONS en un seul batch UPSERT
            to_upsert = [item for item in collected if item.get(pk_field) in added_ids or item.get(pk_field) in modified_ids]

            if to_upsert:
                try:
                    # L'API PostgREST supporte l'upsert via le header 'Prefer: resolution=merge-duplicates'
            ...
            # 3. Traiter les SUPPRESSIONS en un seul batch DELETE (via in.)
            if deleted_ids:
                try:
                    ...
                    # Fallback sur suppression individuelle si le 'in.' est trop long
                    for d_id in deleted_ids:
                        try:
                            self.postgrest.delete(table, {pk_field: f'eq.{d_id}'})
                            results['actions'].append({'type': 'deleted', 'id': d_id})
                        except Exception as e2:
                            results['actions'].append({'type': 'error', 'id': d_id, 'error': str(e2)})
