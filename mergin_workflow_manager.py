...
class MerginDataMerger:
    ...
    def detect_conflicts(self, original, collected, pk_field='id'):
        ...
        # Entrées modifiées (Optimized with dictionary lookup for O(N+M) complexity)
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            changed_ids = modified_ids | added_ids

            # 2. Traiter les ajouts et modifications en batch (UPSERT)
            items_to_upsert = [item for item in collected if item.get(pk_field) in changed_ids]
            if items_to_upsert:
                try:
                    # On utilise insert(upsert=True) pour traiter les ajouts et modifications en une seule fois
                    self.postgrest.insert(table, items_to_upsert, upsert=True)
                    for item in items_to_upsert:
                        item_id = item.get(pk_field)
                        action_type = 'updated' if item_id in modified_ids else 'inserted'
                        results['actions'].append({'type': action_type, 'id': item_id})
                except Exception as e:
                    results['actions'].append({'type': 'error', 'msg': f"Erreur UPSERT batch: {str(e)}"})

            # 3. Traiter les suppressions si nécessaire en batch
            deleted_ids = []
            for c in conflicts:
                if c['type'] == 'deleted':
            ...
            if deleted_ids:
                try:
                    # Utiliser l'opérateur 'in.' pour supprimer en une seule requête
                    id_list = ",".join(str(d_id) for d_id in deleted_ids)
