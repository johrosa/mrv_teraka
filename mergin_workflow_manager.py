...
class MerginDataMerger:
    ...
    def detect_conflicts(self, original, collected, pk_field='id'):
        ...
        # Optimized: dictionary lookup for O(1) access
        ...
        # Entrées modifiées (Optimized with dictionary lookup)
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            # Optimization: Combining inserts and updates reduces network roundtrips significantly.
            ...
            if items_to_upsert:
                try:
                    ...
                    for item in items_to_upsert:
                        ...
                        type_label = 'inserted' if i_id in added_ids else 'updated'
                        results['actions'].append({'type': type_label, 'id': i_id})
                except Exception as e:
                    results['actions'].append({'type': 'error', 'msg': f"Erreur batch UPSERT: {str(e)}"})

            # 3. Traiter les suppressions en batch
            # Optimization: Using the 'in.' operator allows deleting multiple records in one request.
            deleted_ids = []
            for c in conflicts:
                if c['type'] == 'deleted':
