...
class MerginDataMerger:
    ...
    def detect_conflicts(self, original, collected, pk_field='id'):
        ...
        # Entrées modifiées
        orig_indexed = {item.get(pk_field): item for item in original}
        for coll_item in collected:
            item_id = coll_item.get(pk_field)
            orig_item = orig_indexed.get(item_id)
    ...
    def merge(self, table, original, collected, strategy='merge', pk_field='id'):
        ...
        if strategy == 'merge':
            ...
            # PostgREST supporte l'upsert multiple en une seule requête POST avec Prefer: resolution=merge-duplicates
            ...
            # 3. Traiter les suppressions en un seul batch DELETE via l'opérateur in.
            if deleted_ids:
                try:
